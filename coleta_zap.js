...
const ID_OFFSET = 4_000_000_000_000;
...
function row(a, slug) {
  ...
  return {
    list_id: ID_OFFSET + Number(a.id), fonte: "ZAP", coletado_em: new Date().toISOString(),
...
(async () => {
  ...
  for (const [slug, regiao] of BAIRROS) {
    ...
    for (let pg = 1; pg <= PAGES; pg++) {
      const url = `https://www.zapimoveis.com.br/aluguel/imoveis/rj+rio-de-janeiro+${regiao}+${slug}/` + (pg > 1 ? `?pagina=${pg}` : "");
